...
function Learn_Irfs_1(Variables_List,periods,extension)
...
%Exo_Debt = zeros(TSlength,NumberOfStructuralShocks);
%Exo_Debt_frcst = zeros(TSlength,NumberOfStructuralShocks);
%Exo_Debt_learn = zeros(TSlength,NumberOfStructuralShocks);
...
Var = str2mat('Output','Inflation','Interest Rate','Tax Rate','Perceived Target','Perceived Temporary Shock','Total Debt','New Debt','Real Wage','Interest Rate new','Interest Rate D','Spending on Interest','Labor Tax Revenue','Real Interest Rate');
...
% h1 = figure('Name',['Shock to ' Shocks_List(1,:)]);
% 
% istart=1;
% iend = 2;
% for i = istart:iend
...
%     %disp('test2')
%     hold on
%     style1=str2mat('-g','-r','--b');
%     style2=[2,1];
%     h2(1) = plot(y_R_learn(i,2:periods+1),deblank(style1(1,:)),'linewidth',style2(1));
%     %h2(2) = plot(y_R_frcst(i,2:periods+1),deblank(style1(2,:)),'linewidth',style2(2));
%     h2(2) = plot(y_R(i,2:periods+1),deblank(style1(3,:)),'linewidth',style2(1));
%     %TMP = zeros(periods,1);
%     %eval(['TMP(1:periods,:) = irfs_' M_.fname '.' deblank(Variables_List(i,:)) '_ETA_R(1:periods,1);']);
%     %h2(3) = plot(-TMP(1:periods,1),deblank(style1(3,:)),'linewidth',style2(1));
...
%     if i==1
%         legend(h2,'Learning','Full Information','Location','Best'); % ,'Forecast'
%     end
%     hold off
% end
% 
% eval(['print -depsc2 ' 'IRFs_to_' deblank(Shocks_List(1,:)) '_1_' extension]);
% eval(['print -dpdf ' 'IRFs_to_' deblank(Shocks_List(1,:)) '_1_' extension]);
% saveas(h1,['IRFs_to_' deblank(Shocks_List(1,:)) '_1_' extension  '.fig']);
% 
% 
% h3 = figure('Name',['Shock to ' Shocks_List(1,:)]);
% 
% 
% istart= 9;
% iend = 14;
...
%     style1=str2mat('-g','-r','--b');
%     style2=[2,1];
%     h4(1) = plot(y_R_learn(i,2:periods+1),deblank(style1(1,:)),'linewidth',style2(1));
%     %h2(2) = plot(y_R_frcst(i,2:periods+1),deblank(style1(2,:)),'linewidth',style2(2));
%     h4(2) = plot(y_R(i,2:periods+1),deblank(style1(3,:)),'linewidth',style2(1));
%     %TMP = zeros(periods,1);
%     %eval(['TMP(1:periods,:) = irfs_' M_.fname '.' deblank(Variables_List(i,:)) '_ETA_R(1:periods,1);']);
%     %h2(3) = plot(-TMP(1:periods,1),deblank(style1(3,:)),'linewidth',style2(1));
%     plot([1 periods],[0 0],'-k','linewidth',0.5);
...
%     title(deblank(legend_var(i,:)),'Interpreter','none','FontSize',11,'FontWeight','Bold');
%     if i==9
%         legend(h4,'Learning','Full Information','Location','Best'); % ,'Forecast'
%     end
%     hold off
% end
% 
% eval(['print -depsc2 ' 'IRFs_to_' deblank(Shocks_List(1,:)) '_2_' extension]);
% eval(['print -dpdf ' 'IRFs_to_' deblank(Shocks_List(1,:)) '_2_' extension]);
% saveas(h1,['IRFs_to_' deblank(Shocks_List(1,:)) '_2_' extension  '.fig']);
% 
...
for i = istart:iend
    ...
    %TMP = zeros(periods,1);
    %eval(['TMP(1:periods,:) = irfs_' M_.fname '.' deblank(Variables_List(i,:)) '_ETA_PIESTAR(1:periods,1);']);
    %h4(3) = plot(TMP(1:periods,1),deblank(style1(3,:)),'linewidth',style2(1));
    plot([1 periods],[0 0],'-k','linewidth',0.5);
    xlim([1 periods]);
    xlabel('Quarters','FontSize',9);
    title(deblank(legend_var(i,:)),'Interpreter','none','FontSize',11,'FontWeight','Bold');
...
eval(['print -depsc2 ' 'IRFs_to_' deblank(Shocks_List(2,:)) '_Shocks_' extension]);
eval(['print -dpdf ' 'IRFs_to_' deblank(Shocks_List(2,:)) '_Shocks_' extension]);
saveas(h5,['IRFs_to_' deblank(Shocks_List(2,:)) '_Shocks_' extension '.fig']);
...
% saveas(h3,['IRFs_to_' deblank(Shocks_List(2,:)) '_2_' extension '.fig']);