...
%*************************************************************************
	...
	s_e 	= 0;
	s_r 	= 0;
	ln_yd = 0; ln_y = 0; ln_yn = 0; ln_pi = 0; ln_i = 0; ln_ne = 0; ln_rL = 0; ln_l = 0; ln_p = 0; ln_v = 0; ln_c = 0; ln_n = 0; ln_r = 0;
...
%	r-STEADY_STATE(r)= rho*(r(-1)-STEADY_STATE(r)) + (1-rho)*(phi_pi*(pic-1) + phi_pic*(pic-1) +  phi_y*(y-STEADY_STATE(y)) ) + phi_dy*(y-y(-1)) + s_r;
	...
	s_g = rho_g*s_g(-1)+(fiscal_+rho_ag*e_a)/obsFactor;			% spending AR(1) + productivity innovation
...
shocks;
	var e_a;  stderr 0.096;
	var fiscal_;  stderr 0.0232;
	var e_b;  stderr 0.0040;
    var e_i;  stderr 0.0396;
	var e_l;  stderr 0.0018;
	var e_n;  stderr 0.0064;
	var e_p;  stderr 0.0005;
	var e_w;  stderr 0.0027;
	var e_e;  stderr 0.0200;
	var interest_;  stderr 0.09;
end;
...
%stoch_simul(order=1,irf=25) ln_yd ln_c ln_i ln_pi ln_y ln_n ln_v ln_r ln_yn;
%stoch_simul(order=1,irf=25) interest inflation inflationq outputgap output;
