...
var Pih Pif Pih_cpi Pif_cpi Utilh Utilf C Cs Y Ys L Ls S 
    Util Mc Mcs X Xs D Ds Dx Dxs P Ps Px Pxs 
    tb A As Welf omega omegas nfa T Ts eR eRs R Rs
    Pih_f Pif_f Pih_f_cpi Pif_f_cpi Utilh_f Utilf_f C_f Cs_f Y_f Ys_f L_f Ls_f S_f 
...
inflation = (log(Pih_cpi)+ log(Pih_cpi(-1)) + log(Pih_cpi(-2)) + log(Pih_cpi(-3)) );			         //*
inflationq  = 4 * (log(Pih_cpi));                                                      //*
...
//R = (1 / bet) *  (Pih_cpi/Pihtar)^mu * exp(eR);
Rs = (1 / bet) *  (Pif_cpi/Piftar)^mu * exp(eRs);
...
R_f = (1 / bet) *  (Pih_f_cpi/Pihtar)^mu * exp(eR);
Rs_f = (1 / bet) *  (Pif_f_cpi/Piftar)^mu * exp(eRs);

Pih_f_cpi = Pih_f * P_f/P_f(-1);
Pif_f_cpi = Pif_f * Ps_f/Ps_f(-1);
